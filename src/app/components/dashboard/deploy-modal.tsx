              ...
              <div className="px-8 py-8">
                ...
                {(step === 3 && !deploying && !deployed) || (deploying && pipeline.deploy === "pending") ? (
                  <div className="bg-[#fff7ed] border border-[#fed7aa] p-4 mb-6">
                    <div className="font-mono text-[0.55rem] text-[#ea580c] font-medium mb-1">⚠ AI-Generated Code</div>
                    <p className="font-mono text-[0.5rem] text-[#9a3412] leading-relaxed">
                      This contract is generated by AI. Review the source code and security findings before using with real funds. Genesis Engine runs automated security checks but does not replace a professional audit.
                    </p>
                  </div>
                ) : null}
                  ...
                  <div className="mb-6 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="font-mono text-[0.5rem] tracking-[0.15em] uppercase text-ink-soft">
                        Security Review · Score: {securityReport.score}/100
                      ...
                      <span className={`font-mono text-[0.48rem] font-medium ${
                        securityReport.status === "approved" ? "text-[#4ade80]" :
                        securityReport.status === "rejected" ? "text-[#dc2626]" : "text-[#fbbf24]"
                      }`}>
                ...
                {deployed && result ? (
                  <div className="text-center py-8">
                    <motion.div className="w-16 h-16 rounded-full bg-[#4ade80]/10 flex items-center justify-center mx-auto mb-6" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200 }}>
                      <span className="text-2xl text-[#4ade80]">✓</span>
                    </motion.div>
                    <h3 className="font-display font-semibold text-2xl mb-2">Deployment Complete</h3>
                    <p className="font-mono text-[0.56rem] text-ink-soft mb-2">ERC-8004 Agent #{result.id} is live on Mantle</p>
                    <div className="bg-surface-alt border border-border p-4 mb-6 text-left space-y-2 font-mono text-[0.52rem]">
                      ...
                      {securityReport && <div className="flex justify-between"><span className="text-ink-soft">Security</span><span className="text-[#4ade80]">{securityReport.score}/100 · {securityReport.status.toUpperCase()}</span></div>}
                    </div>
                  ...
                  <div className="text-center py-8 space-y-4">
                    ...
                    <p className="font-mono text-[0.56rem] text-ink-soft tracking-[0.08em]">
                      Processing pipeline — AI generation, security review, compilation, deployment...
                    </p>
                  </div>
                ) : step === 1 ? (
                  ...
                  <div className="space-y-5">
                    <div className="bg-surface-alt border border-border p-5 space-y-3 font-mono text-[0.6rem]">
                      ...
                      <div className="flex justify-between"><span className="text-ink-soft">Pipeline</span><span className="text-ink font-medium">Generate → Review → Compile → Deploy → Verify</span></div>
                    </div>
                    <p className="font-mono text-[0.5rem] text-ink-soft tracking-[0.08em] text-center">
                      The AI generates the contract, a security agent reviews it, solc compiles it, and it deploys to Mantle testnet.
                    </p>
